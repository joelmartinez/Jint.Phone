...
namespace Jint.Native {
    ...
    public sealed class JsArray : JsObject {
        ...
        private int FindKeyOrNext(int key) {
            ...
            int index = 0;
            while (left <= right) {
                int current = index;// m_data.Keys[index]; // is this needed? m_data is already keyed by index. I think you can just return key;
                if (current == key)
                    return index;
                else {
                    if (current > key)
        ...
        private int FindKeyOrPrev(int key) {
            ...
            int index = 0;
            while (left <= right) {
                int current = index;// m_data.Keys[index];
                if (current == key)
                    return index;
                else {
                    if (current > key)
        ...
        public override IEnumerable<string> GetKeys() {
            return m_data.Keys.Select(k => k.ToString());
            /*
        ...
        public override IEnumerable<JsInstance> GetValues() {
            foreach (var instance in m_data.Values) yield return instance;
