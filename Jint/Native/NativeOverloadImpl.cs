...
namespace Jint.Native
{
        ...
        where TImpl : class
    {
        ...
        protected TMemberInfo MatchMethod(Type[] args, IEnumerable<TMemberInfo> members)
        {
            ...
            foreach (var m in members)
                matches.AddLast(
                    ...
                    {
                        method = m,
                        parameters = m.GetParameters().Select(p => p.ParameterType).ToArray(),
        ...
        {
            genericArguments = genericArguments ?? new Type[0];
            var ganames = genericArguments.Select(t => t == null ? "<null>" : t.FullName);

            types = types ?? new Type[0];
            var tnames = types.Select(t => t == null ? "<null>" : t.FullName);
        ...
        {
            Type[] argTypes = args.Select( x => m_marshaller.GetInstanceType(x)).ToArray();
