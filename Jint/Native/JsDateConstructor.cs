...
namespace Jint.Native {
    ...
    public class JsDateConstructor : JsConstructor {
        ...
        public JsInstance GetTimezoneOffsetImpl(JsDictionaryObject target, JsInstance[] parameters) {
            return Global.NumberClass.New((DateTime.Now - DateTime.UtcNow).TotalMinutes);
        }
        ...
        public JsInstance UTCImpl(JsInstance[] parameters) {
            ...
            JsDate result = Construct(parameters);
            double offset = result.ToNumber() + (DateTime.Now - DateTime.UtcNow).TotalMilliseconds;// TimeZone.CurrentTimeZone.GetUtcOffset(new DateTime()).TotalMilliseconds;
